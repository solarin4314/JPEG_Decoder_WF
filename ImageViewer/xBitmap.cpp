...
void Init_Bmp(struct XBitmap* xbitmap)
{
	...
	xbitmap->m_rgbSize = 0;					// rgb size : RGB = 3 / RGBA = 4
}
void CreateDib(struct XBitmap* xbitmap, int width, int height, int pixelFormat)
{
	...
	xbitmap->m_rgbSize = pixelFormat / 8;
	xbitmap->m_paddingByte = xbitmap->m_stride - (xbitmap->m_width * xbitmap->m_rgbSize);				// 패딩 바이트 계산
}
...
void WriteBMFile(struct XBitmap* xbitmap)
{
	...
	// 역으로 찍음
	int x, y;
	int bmpDataWidth = xbitmap->m_width * xbitmap->m_rgbSize;

	for (y=xbitmap->m_height-1; y>=0; y--)
	{
		for (x=0; x<bmpDataWidth; x+= xbitmap->m_rgbSize)
		{
		...
		if (((4 - (xbitmap->m_width * xbitmap->m_rgbSize) % 4) % 4)>0)
		{
			unsigned char pad = 0;
			fwrite(&pad, (4 - (xbitmap->m_width * xbitmap->m_rgbSize) % 4), 1, xbitmap->fp);
		}
	}
}
...
int GetRGBSize(struct XBitmap *xbitmap)
{
	if(xbitmap->m_bitSize < 1)
		return (xbitmap->m_width * xbitmap->m_height * xbitmap->m_rgbSize) + ((4 - (xbitmap->m_width * xbitmap->m_rgbSize) % 4) % 4);
	else
...
void MakeBMPHeader(struct XBitmap* xbitmap, const char* bmpFileName)
{
	...
	int iNumPaddedBytes = 4 - (xbitmap->m_width * xbitmap->m_rgbSize) % 4; // 00 padding
